...
module.exports = function ipfilter(ips, opts) {
    ...
    var settings = _.defaults( opts || {}, {
        mode: 'deny',
        log: 'deny',
        logF: function(message){ console.log(message);},
        errorCode: 401,
    ...
    return function(req, res, next) {
        if(settings.excluding.length > 0){
            ...
            if(results.length > 0){
                if(settings.log === 'allow' || settings.log === 'both'){
                    settings.logF({access: 'granted for excluded ip ' + results[0]});
                }
                return next();
            }
        }
        ...
        if(matchClientIp(ip,req)) {
            // Grant access
            if(settings.log === 'allow' || settings.log === 'both') {
                settings.logF({access: 'granted for ' + ip});
            }
        ...
        // Deny access
        if(settings.log === 'deny' || settings.log === 'both') {
            settings.logF({access: 'denied for ' + ip});
        }
